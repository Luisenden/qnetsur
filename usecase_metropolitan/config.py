...
# simulation wrapper function to evaluate configuration
def simwrapper(simulation, kwargs: dict):
    ...
    objectives = mean - max(np.sum(mem_size)-budget, 0) # objective
    objectives_std = std
    raw = mean
    return objectives, objectives_std, raw

# fixed parameters of quantum network simulation
vals = {
...
# variables and bounds of quantum network simulation
vars = {
        'range':{},
        'choice':{},
        'ordinal':{}
